...
const fileType = (input: ArrayBuffer): {ext: string; mime: string} | null => {
  ...
  const check = (header: Array<number>, opts?: {offset: number}): boolean => {
    opts = Object.assign({
      offset: 0,
    }, opts)
  ...
  if (check([0x1A, 0x45, 0xDF, 0xA3])) {
    ...
    if (idPos >= 0) {
      ...
      const findDocType = (type: string): boolean => Array.from(type).every((c, i) => sliced[docTypePos + i] === c.charCodeAt(0))
...
export function getFileType(file: File): Promise<{ext?: string; mime?: string} | null> {
